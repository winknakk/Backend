...
assert.equal(L("AppSup Test", "IN_PROGRESS"), "WAITING_INTERNAL", "AppSup test maps to internal testing");
assert.equal(L("Test Failed", "TRIAGED"), "IN_PROGRESS");
assert.equal(L("Test Failed", "IN_PROGRESS"), null, "already being fixed");
assert.equal(L("Waiting for Customer", "IN_PROGRESS"), "RESOLVED", "waiting for customer test invites confirmation");
assert.equal(L("Waiting for Customer", "RESOLVED"), null);
assert.equal(L("Customer Test", "IN_PROGRESS"), "RESOLVED", "customer test asks the customer to confirm");
