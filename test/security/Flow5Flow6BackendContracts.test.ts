 ...
 * K. Flow 5 cancel transition to CANCELLED is idempotent
 */
...
async function customerSession(ref: string, projectId: number) {
  ...
  return { token: body.token, claims, identityId: ident.rows[0]?.id };
}
...
describe("Flow 5 & 6 Backend Contract Closure", () => {
  before(async () => {
    // 1. Pick two real projects
    const projRes = await pool.query("SELECT id FROM projects ORDER BY id ASC LIMIT 2");
    assert.ok(projRes.rows.length >= 2, "Must have at least 2 projects in database");
    ...
    // 3. Create Ticket A and Ticket B for Customer 1 under Project 1
  ...
  it("G: Inbound WebChat message persists messages.ticket_id under authorized active ticket", async () => {
    ...
    const ackPromiseA = new Promise<any>((resolve) => {
      ws.on("message", (raw) => {
        try {
    ...
    const ackPromiseB = new Promise<any>((resolve) => {
      ws.on("message", (raw) => {
        try {
          const p = JSON.parse(raw.toString());
  ...
  it("J & K: Flow 5 Cancel confirmation transitions to CANCELLED idempotently and closed ticket is not restored", async () => {
    // Transition Ticket B to CANCELLED
    const cancelRes1 = await fetch(`${API}/api/portal/tickets/${ticketB.id}/transition`, {
      method: "POST",
      headers: { "content-type": "application/json", authorization: `Bearer ${tokenCust1}` },
      body: JSON.stringify({ targetStatus: "CANCELLED", reason: "Customer requested cancel" }),
    });
    assert.equal(cancelRes1.status, 200, "Customer cancellation must succeed");
    const cancelBody1 = await cancelRes1.json() as any;
    assert.equal(cancelBody1.to, "CANCELLED");

    // Duplicate cancellation request MUST be idempotent (Exit 200)
    const cancelRes2 = await fetch(`${API}/api/portal/tickets/${ticketB.id}/transition`, {
      method: "POST",
      headers: { "content-type": "application/json", authorization: `Bearer ${tokenCust1}` },
      body: JSON.stringify({ targetStatus: "CANCELLED", reason: "Duplicate cancel" }),
    });
    assert.equal(cancelRes2.status, 200, "Duplicate cancellation must be idempotent with 200 OK");

    // J: Verify conversations.active_ticket_id was cleared upon cancellation
    const { rows } = await pool.query("SELECT active_ticket_id FROM conversations WHERE id = $1", [convId]);
