...
async function run() {
  console.log("Querying identities table columns from NocoDB...");
  try {
    const res = await axios.get(`${baseUrl}/api/v1/db/data/v1/${baseId}/${tableIdentities}?limit=1`, {
      headers: { "xc-token": apiToken },
    });
    const list = res.data?.list || res.data || [];
    if (list.length > 0) {
      console.log("Keys in identities row:", Object.keys(list[0]));
