...
async function run() {
  ...
  console.log("1. Querying conversation with identity_id only...");
  try {
  ...
  console.log("\n2. Querying conversation with identity_id and status using ~and...");
  try {
    const res = await axios.get(`${baseUrl}/api/v1/db/data/v1/${baseId}/${tableConversations}`, {
      headers: { "xc-token": apiToken },
      params: {
        where: `~and((identity_id,eq,${identityId}),(status,eq,open))`,
        limit: 1,
      }
    });
    console.log("Success! Rows:", res.data.list.length);
